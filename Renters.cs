...
namespace ComicRentalSystem
{
...

    {
        ...
        private void btnFetch_Click(object sender, EventArgs e)
        {
            ...
            string query = "SELECT * FROM Renters WHERE CustomerID LIKE @text OR Name LIKE @text "; // new
            ...
            using (connection = new SqlConnection(connectionString))
            {
                ...
                if (reader.Read())
                {
                    ...
                    txtBxDetails.Text = id + Environment.NewLine + name + Environment.NewLine + email + Environment.NewLine + status; // new
                }
                else
                {
        ...
        {
            string checkStatus = "SELECT Status FROM Renters WHERE CustomerID LIKE @id OR Name LIKE @id";
            connection = new SqlConnection(connectionString);
            ...
            using(SqlCommand command = new SqlCommand(checkStatus,connection))
            {
                ...
                if(result == null)
                {
                    MessageBox.Show("Record returns nothing.");
                    return;
                }
                ...
                if (status == "Banned    ")
                {
                    Console.WriteLine(status);
                        string query = "UPDATE Renters SET Status = 'Active' WHERE CustomerID LIKE @id OR Name LIKE @id" ;
